...
if [ ! -f ".next/standalone/server.js" ] || ! head -1 .next/standalone/server.js | grep -q "path"; then
  ...
  if [ -f "$NESTED" ]; then
    ...
    # CRITICAL: use -rL (dereference symlinks) not -a (preserves symlinks → broken on server)
    NESTED_DIR=$(dirname "$NESTED")
    for item in "$NESTED_DIR"/* "$NESTED_DIR"/.[!.]*; do
      [ -e "$item" ] || continue
      base=$(basename "$item")
      if [ "$base" != "server.js" ]; then
        cp -rL "$item" .next/standalone/
      fi
    done
  fi
fi
...
# --- Scripts (sync-onboarding-to-profile.js standalone) ---
mkdir -p deploy-dist/scripts
cp scripts/sync-onboarding-to-profile.js deploy-dist/scripts/
# Generate production prisma.config.js (CommonJS, zero external deps — no prisma/config or dotenv needed on VPS)
cat > deploy-dist/prisma.config.js <<'PRISMA_CONFIG'
// Production prisma config — generated by prepare-deploy.sh
// CommonJS format avoids needing prisma/config or dotenv in node_modules on the VPS.
// Environment variables are sourced from .env via `source .env` before running prisma CLI.
module.exports = {
...
for required_path in \
  ...
  "deploy-dist/prisma.config.js" \
  "deploy-dist/scripts/sync-onboarding-to-profile.js" \
  "deploy-dist/logs"; do
