...
- [ ] [Review][Patch] Non-authors receive legal document metadata on the deal detail page [src/app/(dashboard)/opportunities/[id]/page.tsx:45, src/app/(dashboard)/opportunities/[id]/page.tsx:56, src/components/features/deals/document-upload-section.tsx:246] — AC4 and the story security constraints require document access/preview/download to be restricted to the opportunity author or an ADMIN, with other members seeing at most a counter. The detail page currently includes full `documents` for every premium viewer, serializes `originalName`, `fileName`, `mimeType`, `size`, and timestamps, then renders rows even when `canPreview={false}`. This exposes sensitive legal document names and metadata to unauthorized members.
- [ ] [Review][Patch] Upload completion trusts arbitrary client-provided R2 keys [src/app/api/opportunities/[id]/documents/route.ts:47, src/app/api/opportunities/[id]/documents/route.ts:55] — The completion endpoint validates file name/type/size but accepts any `r2Key` supplied by the client and stores it/signs it later. This breaks the signed-upload trust boundary: an authorized author/admin for one opportunity can create DB rows for keys not generated by `/presign-url` or not under `opportunities/{id}/documents/`, potentially causing the app to sign arbitrary bucket objects if a key is known. Constrain completion to server-generated keys (prefix + UUID/extension pattern at minimum, preferably a short-lived pending-upload token/record) and ensure `fileName` matches the key basename.

### Acceptance Audit

- AC1: Partially satisfied. Signed PUT URLs, file type validation, size limit, and `DocumentRow` exist, but the completion endpoint does not bind completion to a server-issued presign/key.
- AC2: Mostly satisfied for existing opportunities via XHR progress and non-blocking state. Creation-flow pending uploads happen after opportunity creation without visible progress in `uploadPendingLegalDocuments`; not raised as blocker because files are staged before submit.
- AC3: Satisfied. Detail/list counters and document section are implemented.
- AC4: Failed. Non-author premium viewers receive document metadata through the detail page even though preview/download buttons are hidden; API preview/download/list routes enforce author/admin.

### Verification Run

- `npm run build`: passed.
- `npx vitest run`: passed (209 tests).
- `npx prisma validate`: `npx prisma validate` failed in this shell with an `rtk` shim error; direct `./node_modules/.bin/prisma validate` passed.
- Sprint/story status updated to `in-progress` because unresolved review patch findings remain.
