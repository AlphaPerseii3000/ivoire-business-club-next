...
- [ ] Task 1 : Audit du code existant et delta-scoping (AC: #1, #2)
  - [ ] 1.1 Lire `src/app/(admin)/admin/subscriptions/page.tsx` — tableau de bord admin actuel (sections "Virements en attente" / "Abonnements actifs")
  - [ ] 1.2 Lire `src/components/admin-subscription-actions.tsx` — boutons Valider/Refuser/Suspendre existants
  - [ ] 1.3 Lire `src/app/api/admin/subscriptions/[id]/route.ts` — logique de transition PENDING/ACTIVE et audit log existante
  - [ ] 1.4 Lire `src/app/api/admin/subscriptions/[id]/route.test.ts` — patterns de tests et mocks attendus pour audit log
  - [ ] 1.5 Lire `src/components/payment-method-selector.tsx` et `src/lib/mobile-money-config.ts` — icônes/couleurs Wave/Orange Money déjà utilisées
  - [ ] 1.6 Lire `src/app/api/subscriptions/route.ts` et `src/lib/validations.ts` — confirmer que WAVE/ORANGE_MONEY créent un abonnement en `TRIAL` avec `providerPhone`
  - [ ] 1.7 Identifier la divergence de statut : virement bancaire = `PENDING`, mobile money = `TRIAL` (état dans lequel l'admin valide)

- [ ] Task 2 : Adapter l'API admin pour accepter la transition TRIAL → ACTIVE (AC: #2)
  - [ ] 2.1 Modifier `src/app/api/admin/subscriptions/[id]/route.ts` pour autoriser `validate` depuis `PENDING` **OU** `TRIAL` (les deux providers sont validables manuellement)
  - [ ] 2.2 Conserver le rejet/refus depuis `PENDING` ou `TRIAL` vers `CANCELLED` (même workflow que virement)
  - [ ] 2.3 Conserver `suspend` uniquement depuis `ACTIVE`
  - [ ] 2.4 Conserver la transaction Prisma : `subscription.status` + `payment.status` mis à jour ensemble
  - [ ] 2.5 L'audit log doit être créé AVANT l'envoi d'email (ordre actuel déjà correct, le garder)
  - [ ] 2.6 Inclure `provider` (BANK_TRANSFER/WAVE/ORANGE_MONEY) dans les métadonnées de l'audit log

- [ ] Task 3 : Afficher le provider et l'icône distinctive dans le tableau admin (AC: #1)
  - [ ] 3.1 Modifier `src/app/(admin)/admin/subscriptions/page.tsx` pour afficher une colonne "Moyen de paiement" avec le label et l'icône
  - [ ] 3.2 Utiliser `getProviderColorClasses` de `src/lib/mobile-money-config.ts` pour les couleurs Wave (bleu) / Orange Money (orange)
  - [ ] 3.3 Pour `BANK_TRANSFER`, afficher une icône `Landmark` et un badge neutre (comportement existant conservé)
  - [ ] 3.4 Afficher `providerPhone` sous la référence pour les abonnements WAVE/ORANGE_MONEY (aide à la validation)
  - [ ] 3.5 Adapter les libellés des sections si nécessaire : "Abonnements à valider" au lieu de "Virements en attente" (optionnel, rester cohérent)

- [ ] Task 4 : Créer un composant réutilisable `PaymentProviderBadge` (AC: #1)
  - [ ] 4.1 Créer `src/components/payment-provider-badge.tsx`
  - [ ] 4.2 Props : `provider: PaymentProvider`, `providerPhone?: string | null`, `showPhone?: boolean`
  - [ ] 4.3 Retourner un badge coloré avec icône + label pour chaque provider
  - [ ] 4.4 Utiliser les couleurs : Wave = bleu, Orange Money = orange, Bank Transfer = gris/slate
  - [ ] 4.5 Utiliser `Waves` et `Smartphone` de `lucide-react`, et `Landmark` pour virement
  - [ ] 4.6 Exporter `PaymentProvider` type depuis ce composant ou réutiliser celui de `mobile-money-config.ts`

- [ ] Task 5 : Adapter `AdminSubscriptionActions` si nécessaire (AC: #2)
  - [ ] 5.1 Vérifier que le composant envoie bien `{ action: "validate" }` et gère `TRIAL` comme état actionnable
  - [ ] 5.2 Si l'API retourne une erreur de transition, afficher le message dans le toaster
  - [ ] 5.3 Aucun changement visuel majeur requis ; les labels Valider/Refuser/Suspendre restent identiques

- [ ] Task 6 : Tests unitaires et d'intégration (AC: #1, #2)
  - [ ] 6.1 Mettre à jour `src/app/api/admin/subscriptions/[id]/route.test.ts` :
    - test : validation depuis `TRIAL` (provider = WAVE) → `ACTIVE` + payment `succeeded` + email envoyé + audit log créé avec `provider: "WAVE"`
    - test : validation depuis `TRIAL` (provider = ORANGE_MONEY) → `ACTIVE` + audit log avec `provider: "ORANGE_MONEY"`
    - test : rejet depuis `TRIAL` (provider = WAVE) → `CANCELLED` + payment `failed` + email de refus + audit log créé
    - test : `validate` depuis `ACTIVE` reste interdit (409)
    - test : `suspend` depuis `TRIAL` reste interdit (409)
    - test : mock `safeCreateAuditLog` est appelé avec `action`, `entityType: "Subscription"`, `entityId: "sub-1"`, et métadonnées incluant `provider`
  - [ ] 6.2 Mettre à jour `src/app/(admin)/admin/subscriptions/page.test.tsx` :
    - test : un abonnement TRIAL WAVE affiche le badge bleu et le numéro mobile money
    - test : un abonnement TRIAL ORANGE_MONEY affiche le badge orange
    - test : un abonnement PENDING BANK_TRANSFER affiche l'icône virement
  - [ ] 6.3 Créer `src/components/payment-provider-badge.test.tsx` :
    - test : WAVE rend un élément avec texte "Wave" et couleur bleue
    - test : ORANGE_MONEY rend "Orange Money" et couleur orange
    - test : BANK_TRANSFER rend "Virement bancaire"
    - test : `providerPhone` affiché quand `showPhone=true`
  - [ ] 6.4 Mettre à jour `src/components/admin-subscription-actions.test.tsx` si un nouveau guard est ajouté

- [ ] Task 7 : Vérifications finales
  - [ ] 7.1 Lancer `npm run test` (Vitest) : tous les tests passent
  - [ ] 7.2 Lancer `npm run build` : build Next.js passe sans erreur TypeScript
  - [ ] 7.3 Vérifier qu'aucun `&&` n'est utilisé en expression JSX (ternaires obligatoires)
  - [ ] 7.4 Vérifier que les routes admin ont bien les guards `role === ADMIN` ET `status !== SUSPENDED` (via `AdminLayout` déjà existant)
