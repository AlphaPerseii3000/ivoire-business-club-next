...
- [x] [Review][Patch] Implement the specified `GET /api/admin/subscriptions` route; the current build exposes only `/api/admin/subscriptions/[id]`, so the listed admin subscriptions API contract is missing [src/app/api/admin/subscriptions/route.ts:1] — FIXED: route added
- [x] [Review][Patch] Replace the new boolean `&&` JSX conditional with a ternary returning `null` to satisfy the Next.js 16 strict project guardrail [src/components/admin-subscription-actions.tsx:69] — FIXED: ternary applied
